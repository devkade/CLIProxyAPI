...
func (cfg *Config) ValidateCredentialWeights() error {
	...
	for index := range cfg.OllamaCloudKey {
		if errValidate := ValidateCredentialWeight(cfg.OllamaCloudKey[index].Weight); errValidate != nil {
			return fmt.Errorf("ollama-cloud-api-key[%d].weight: %w", index, errValidate)
		}
	}
