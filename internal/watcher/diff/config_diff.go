...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.MaxCooldownSeconds != newCfg.MaxCooldownSeconds {
		changes = append(changes, fmt.Sprintf("max-cooldown-seconds: %d -> %d", oldCfg.MaxCooldownSeconds, newCfg.MaxCooldownSeconds))
	}
